...
bfs(node_t* tree) {
  ...
  while (!q.empty()) { // iterate over q
     node_t *tmp = q.front(); q.pop();// pop q

     if (tmp == NULL) { // if level marker
			 cout << endl; // print new line indicating begining of new level
       if (q.empty()) return; // after previous pop if q is empty return
		   q.push(NULL); // push new marker
       tmp = q.front(); q.pop();// retrive q for processing down
     }
