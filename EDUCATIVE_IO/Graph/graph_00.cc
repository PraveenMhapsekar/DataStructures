...
int
choose (int *dist, int n, int *visited) {
  int i, min, minpos;
  min = INT_MAX;
  minpos = -1;
...
void
dikjastra(graph_t *G, int v, int *dist, int *visited) {
  int i, u, w;
  int n = G->nVertices; 
  ...
  for (i = 0; i < (n - 2); i++) {
    /* Chose minimum unvisited vertext from dist[] */
    u = choose(dist, n, visited);
    for (w = 0; w < n; w++) {
      if (!visited[w]) {
         /* If cost of reaching vertext w from u is less than known cost of reaching w , 
          update dist[w] with new cost to reach w */
        if ((dist[u] + getCost(G, u, w)) < dist[w]) {
          dist[w] = dist[u] + getCost(G, u, w);  
        }
      }
    } /* end of w-loop */
    printDistance();
...
main (void)
{
  ...
  V = 4;
  printHeader(V);
  dikjastra(G, V, dist, visited); /* Dikjastra's Algorithm */
